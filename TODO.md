...
## ✅ Phase 11：批量处理（2026-02-21 完成）

- [x] `src/batch_processor.py` 批量任务队列（逐个处理，无额外 API 费用）
- [x] CSV 导入（格式：产品名称,卖点1,卖点2,...）+ 下载 CSV 模板
- [x] 手动添加产品列表（最多 20 个）
- [x] 队列顺序处理（后台线程，自动写入历史记录）
- [x] 批量下载 ZIP（`GET /api/batch/{id}/download`）
- [x] `static/batch.html` 批量处理页面（产品列表 + 进度表 + ZIP 下载）
