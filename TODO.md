...
**整体进度**: 65% (MVP + Web 界面完成，正在开发 Seedance 集成)
**更新时间**: 2026-02-20
...
| **当前阶段** | ✅ MVP + Web 界面完成，下一步：Seedance 集成 |
...
## 🔴 Phase 7：豆包 Seedance 集成（下一步，P0）

> **背景**: Seedance 是字节跳动旗下豆包大模型的视频生成服务，通过火山引擎 ARK 平台调用。
> 国内可直连，成本约 ¥0.14/次（比 Creatok 便宜 10 倍），支持图生视频。

### 7.1 后端：Seedance 生成器
- [ ] `src/config.py` 新增配置项
  - [ ] `ARK_API_KEY`（火山引擎 API Key）
  - [ ] `SEEDANCE_MODEL_ID`（模型 ID）
  - [ ] `SEEDANCE_API_URL`（ARK 接口地址）
- [ ] `src/video_generator.py` 新增 `generate_with_seedance()` 方法
  - [ ] 调用火山引擎 ARK 接口（图生视频模式）
  - [ ] 传入白底图作为参考帧
  - [ ] 轮询任务状态
  - [ ] 下载生成的视频
...
### 7.2 后端：API 更新
- [ ] `src/api_server.py` 的 `/api/generate` 新增 `video_service` 参数
  - [ ] 支持 `seedance`（默认） / `creatok` 两种选项
  - [ ] 根据参数调用不同生成器

### 7.3 前端：服务选择器
- [ ] `static/index.html` 新增视频服务选择区域
  - [ ] 单选按钮：豆包 Seedance（推荐）/ Creatok
  - [ ] 显示各服务的简介和成本对比
  - [ ] 提交时传入 `video_service` 字段
...
| Phase 7 | **Seedance 集成** | 4-6h | **P0** | 🔴 待开发 |
...
| 问题 | 文件 | 说明 |
|------|------|------|
| Creatok 参考图上传未实现 | `src/video_generator.py:73` | 有 TODO 占位，传图逻辑缺失 |
...
| ARK_API_KEY | 豆包 Seedance | ⏳ 新增，需申请 |
...
*最后更新: 2026-02-20（新增 Seedance 规划，重新规划路线图）*
